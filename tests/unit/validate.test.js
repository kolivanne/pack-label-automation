...
try {
  ...
  assert.strictEqual(
    resultInvalidData.isValid,
    false,
    "Should fail when flavor is missing",
  );
  assert.ok(
    resultInvalidData.errors.includes("Missing flavor"),
    "Error should mention missing flavor",
  );
  ...
  assert.strictEqual(
    resultEmptyClaims.isValid,
    true,
    "Should still be valid without claims",
  );
  assert.ok(
    resultEmptyClaims.warnings.length > 0,
    "Should produce a warning for missing claims",
  );
